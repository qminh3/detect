...
class DetectionAnalyzer:
    ...
    def analyze_detections(self, detections):
        ...
        for detection in detections:
            ...
            if zones_containing:
                for _, zone in zones_containing:
                    zone_name = zone['name']
    ...
    def draw_analysis_results(self, image, detections, analysis):
        for detail in analysis['detection_details']:
            ...
            # fallback an toàn: nếu không có class_name, dùng ID
            class_name = detection.get('class_name', f"ID-{detection.get('track_id', 'N/A')}")
            confidence = detection.get('confidence', 1.0)

            if is_danger:
                color = (0, 0, 255)
                thickness = 3
                cv2.putText(image, f"DANGER! {class_name}",
                            (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX,
                            0.8, color, thickness)
                if zones:
                    zone_name = zones[0][1]['name']
                    cv2.putText(image, f"IN {zone_name}",
    ...
    def print_analysis_report(self, analysis):
        ...
        print(f"Danger detections: {analysis['danger_detections']}")
        print(f"Outside detections: {analysis['outside_detections']}")
        print("\nZone counts:")
        for zone_name, count in analysis['zone_detections'].items():
            print(f"  - {zone_name}: {count}")
        if analysis['danger_detections'] > 0:
            print("\n⚠️  WARNING: Object(s) in danger zone!")
        print("=" * 50)
